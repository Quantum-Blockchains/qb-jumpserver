...
local base = BaseModel.new("ssh_services", "id")

_M.create_ssh_service = function(service_data)
    return base:create(service_data)
end

_M.update_ssh_service = function(service_id, service_data)
    return base:update(service_id, service_data)
end

_M.delete_ssh_service = function(service_id)
    return base:delete(service_id)
end

_M.get_ssh_service = function(service_id)
...
_M.get_all_ssh_services = function()
...
_M.get_enabled_ssh_services = function()
...
_M.exists_ssh_service = function(service_id)
    return base:exists(service_id)
end

_M.count_ssh_services = function(where_clause, params)
    return base:count(where_clause, params)
end

_M.search_ssh_services = function(search_term)
    local search_fields = {"name", "description", "host", "username"}
    return base:search(search_term, search_fields, "name ASC")
end

-- SSH-specific operations
function _M.import_ssh_services_from_json(import_data)
...
function _M.export_ssh_services_to_json()
...
function _M.get_ssh_service_stats(service_id)
    local service = base:get(service_id)
    if not service then
        return nil, "SSH service not found"
    end
...
-- Validation methods
_M.validate_ssh_service = function(service_data)
    ...
    if not valid_port then
        ...
        if service_data.username and type(service_data.username) == "string" then
        if service_data.username:match("^[%w%-_]+$") == nil then
...
-- Sanitization
_M.sanitize_ssh_service = function(service_data)
    local allowed_fields = {
        id = true, name = true, type = true, host = true, port = true,
        username = true, enabled = true, description = true, source = true
    }
